...
psp_fmgr_getExtId(const char *szFilePath) 
{
  ...
  if((pszExt = strrchr(szFilePath, '.'))) {
    if (!strcasecmp(pszExt, ".pce")) {
      return FMGR_FORMAT_ROM;
    } else
    if ((!strcasecmp(pszExt, ".bin")) ||
        (!strcasecmp(pszExt, ".hcd")) ||
        (!strcasecmp(pszExt, ".toc")) ||
        (!strcasecmp(pszExt, ".isq")) ||
        (!strcasecmp(pszExt, ".iso"))) {
      return FMGR_FORMAT_CD;
...
psp_fmgr_menu(int format)
{
  ...
  if (first) {
    first = 0;
    getcwd(user_filedir_kbd, GP2X_FMGR_MAX_NAME);
  ...
  if (format == FMGR_FORMAT_CD ) user_filedir = user_filedir_cd;
