...
psp_menu_display_save_name()
{
  char buffer[128];
  // int Length;
  snprintf(buffer, 120, "%s", HUGO.hugo_save_name);
...
psp_main_menu_reset(void)
{
  ...
  psp_sdl_back2_print( 90, 110, "Reset Hu-Go !", PSP_MENU_WARNING_COLOR);
  psp_sdl_flip();
  hugo_emulator_reset();
  sleep(1);
}
  ...
  while (! end_menu)
  {
    ...
    if ((new_pad == GP2X_CTRL_CIRCLE))
    {
      switch (cur_menu_id ) 
      {
        ...
        // case MENU_LOAD_CD   : if (psp_main_menu_load(FMGR_FORMAT_CD)) {
        //                         end_menu = 1;
        //                       }
        //                       old_pad = new_pad = 0;
        // break;              
