...
enum {
  ...
  static int hugo_auto_fire_period = 0;
  static int hugo_auto_fire_mode   = 0;
  static int hugo_auto_fire_button = 0;
...
{
  if (step > 0) {
    if (hugo_auto_fire_period < 19) hugo_auto_fire_period++;
  } else {
    if (hugo_auto_fire_period >  0) hugo_auto_fire_period--;
  }
}
...
psp_display_screen_joystick_menu(void)
{
  ...
  for (menu_id = 0; menu_id < MAX_MENU_JOY_ITEM; menu_id++) {
    ...
    if (menu_id == MENU_JOY_AUTOFIRE_T) {
      sprintf(buffer,"%d", hugo_auto_fire_period+1);
      string_fill_with_space(buffer, 7);
      psp_sdl_back2_print(140, y, buffer, color);
    } else
    if (menu_id == MENU_JOY_AUTOFIRE_M) {
      if (hugo_auto_fire_mode) strcpy(buffer,"yes");
      else                    strcpy(buffer,"no ");
      string_fill_with_space(buffer, 4);
      psp_sdl_back2_print(140, y, buffer, color);
    } else
    if (menu_id == MENU_JOY_AUTOFIRE_B) {
      if (hugo_auto_fire_button) strcpy(buffer,"B ");
      else                    strcpy(buffer,"A ");
      string_fill_with_space(buffer, 4);
      psp_sdl_back2_print(140, y, buffer, color);
      y += y_step;
    } else
...
{
  psp_reverse_analog    = HUGO.psp_reverse_analog;
  psp_active_joystick   = HUGO.psp_active_joystick;
  hugo_auto_fire_period = HUGO.hugo_auto_fire_period;
  hugo_auto_fire_mode   = HUGO.hugo_auto_fire;
  hugo_auto_fire_button = HUGO.hugo_auto_fire_button;
}
...
{
  /* Validate */
  HUGO.psp_reverse_analog  = psp_reverse_analog;
  HUGO.psp_active_joystick = psp_active_joystick;
  HUGO.hugo_auto_fire_period = hugo_auto_fire_period;
  if (hugo_auto_fire_mode != HUGO.hugo_auto_fire) {
    kbd_change_auto_fire(hugo_auto_fire_mode);
  }
  if (hugo_auto_fire_button != HUGO.hugo_auto_fire_button) {
    kbd_change_auto_fire_button(hugo_auto_fire_button);
  }
}
  ...
  while (! end_menu)
  {
    ...
    if ((new_pad == GP2X_CTRL_LEFT ) || (new_pad == GP2X_CTRL_RIGHT))
    {
      ...
      switch (cur_menu_id ) 
      {
        ...
        break;              
        case MENU_JOY_AUTOFIRE_M  : hugo_auto_fire_mode = ! hugo_auto_fire_mode;
        break;              
        case MENU_JOY_AUTOFIRE_B  : hugo_auto_fire_button = ! hugo_auto_fire_button;
        break;              
