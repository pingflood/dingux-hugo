...
psp_keyboard_menu_mapping(int kbd_id, int step)
{
  ...
  if (ploc_kbd_mapping[kbd_id] <  -3) ploc_kbd_mapping[kbd_id] = HUGO_MAX_KEY-1;
  else
  if (ploc_kbd_mapping[kbd_id] >= HUGO_MAX_KEY) ploc_kbd_mapping[kbd_id] = -3;
}
      ...
      if (danzeff_key > DANZEFF_START) {
        ...
        if (hugo_key != -1) {
              ...
              (cur_menu_id <= MENU_KBD_JOY_RIGHT)) 
          {
            ...
            if (menu_kbd_selected == -1) loc_kbd_mapping[kbd_id] = hugo_key;
            else
            if (menu_kbd_selected == KBD_LTRIGGER_MAPPING) loc_kbd_mapping_L[kbd_id] = hugo_key;
            else
            if (menu_kbd_selected == KBD_RTRIGGER_MAPPING) loc_kbd_mapping_R[kbd_id] = hugo_key;
          }
        }
