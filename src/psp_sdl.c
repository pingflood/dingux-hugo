...
psp_sdl_save_screenshot(void)
{
  ...
  sprintf(TmpFileName,"%s/scr/screenshot_%d.png", HUGO.hugo_home_dir, HUGO.psp_screenshot_id++);
  if (HUGO.psp_screenshot_id >= 10) HUGO.psp_screenshot_id = 0;
...
psp_sdl_init(void)
{
  ...
  for (Index = 0; Index < HUGO_MAX_SAVE_STATE; Index++) {
    HUGO.hugo_save_state[Index].surface = 
